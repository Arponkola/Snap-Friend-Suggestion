...
import os
from recommender import TwitterRecommender, jaccard_similarity
...
@st.cache_resource
def load_data():
    graph_path = 'C:\\Users\\HI\\Desktop\\ProjectX\\models\\twitter_graph.gpickle'
    model_path = 'C:\\Users\\HI\\Desktop\\ProjectX\\models\\twitter_node2vec.model'
    ...
    if not os.path.exists(GRAPH_FEATURE_MAP_PATH):
        return None, None, None
    
    with open(GRAPH_FEATURE_MAP_PATH, 'rb') as f:
        mapping_feature = pickle.load(f)
    
    rec_engine = TwitterRecommender(model_path)
    return G, rec_engine, dict(mapping_feature)

G, rec_engine, mapping = load_data()
